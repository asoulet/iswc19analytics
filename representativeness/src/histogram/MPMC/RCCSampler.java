...
public class RCCSampler extends TriplestoreSampler {

	private static Logger logger = Logger.getLogger(RCCSampler.class);
	...
	public RCCSampler(String filename) {
	...
	public long getWeight(Triplestore triplestore) {
		weight = 0;
		try {
			SimpleSparqlQuerier q = new SimpleSparqlQuerier("select (count(*) as ?count) where {?s <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> ?o}", triplestore) {
